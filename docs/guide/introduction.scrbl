...
in. Xiden uses Racket to solve this problem.

In the context of Xiden, a @deftech{package} is an actively
...
symbolic links as references, Xiden can detect if files are eligible
for garbage collection.

Overall, Xiden is a viable piece of a continuous integration system
...
instructions. Another change is that Xiden defines no collections in a
...
build. @secref{workspace} covers the directory used to store Xiden's
files.
...
Xiden specifically so that I could release new code without worrying
about how people have set up their Racket installations.

This means that Xiden is not meant to be viewed as a competitor to
@tt{raco pkg}, but a way to complement @tt{raco pkg}'s functionality. It is not
possible to change @tt{raco pkg} to do what Xiden does without breaking
backwards compatibility, and adding another mode to @tt{raco pkg} would
complicate its interface.

That being said, Xiden still depends on packages made available by
@tt{raco pkg}. In fact, Xiden can be installed using @tt{raco pkg} as
...
@section{Why use Xiden when Guix/Nix exists?}

First, Guix and Nix do not support Windows. Xiden does.

Second, Xiden offers a path to leveraging language-oriented programming
