#lang scribble/manual

@require["../shared.rkt" @for-label[racket/base]]

@title{Concepts}

A simple way to maintain dependencies is to check them all into source
control. Many programmers do not do this for obvious reasons, but it
raises the issue of how you can reproduce dependencies as if they
@italic{were} checked in. @project-name uses Racket as a build
environment to solve this problem.

In the context of @|project-name|, a @deftech{package} is a program.
Dependencies, setup instructions, scripts, and the output of
@italic{other} packages are all @tech{inputs} to a package. The
outputs of a package are content-addressable files built from the
inputs within a transaction. Users bind symbolic links to outputs to
use in their own projects.

This approach has several benefits. It makes a package installation
atomic, meaning that an installation failure does not impact your
Racket installation or the wider system. You can override package
inputs like you would with @racket[parameterize].  This gives you a
way to leave out artifacts like docs or tests, or to replace a
dependency without depending on the package's maintainer. Using
symbolic links as references also creates a way to detect if files are
eligible for garbage collection.

@secref{new-pkg} covers how to write @deftech{package definitions}
using the @racketmodname[xiden] language. @secref{workspace} covers
the directory used to store @|project-name|'s files.

@project-name ships with a zero-trust configuration, meaning it may
come off as bereaucratic and/or paranoid when facing decisions that
can impact your system. @secref{config} covers how to configure
@binary to strike your own balance between security and convenience,
but it would be best to learn how to work with the zero-trust
configuration when using code from the Internet.
